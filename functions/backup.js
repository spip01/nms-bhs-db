...
function main() {
    ...
    ref.listDocuments().then(refs => {
        refs.forEach(async ref => {
            let g = await admin.firestore().doc("stars6/" + ref.id).get().then(async doc => {
                if (doc.exists)
                    return doc.data()
                else
                ...
                refs.forEach(ref => {
                    ...
                    ref.get().then(snapshot => {
                        snapshot.docs.forEach(doc => {
                            let path = doc.ref.path.replace(/stars5\/(.*)/, "stars6/$1")
                            let ref = admin.firestore().doc(path)
                            // ref.set(doc.data())
                        })

                        let path = ref.path.replace(/stars5\/(.*?)\/.*/, "stars6/$1")
                        // admin.firestore().doc(path).update({
                        //     backup: admin.firestore.FieldValue.serverTimestamp()
                        // })
                    })
                })
            })
